// Orignally created by Victor saar
...
bool CInput::UnAcquireMouse(void)
{
	if(FAILED(m_pDirectInputMouseDevice->Unacquire()))
	 return false;

}

bool CInput::AcquireKeboard(void)
{
  if(FAILED(m_pDirectInputKeyboardDevice->Acquire()))
	 return false;
}

bool CInput::UnAcquireKeboard(void)
{
	if(FAILED(m_pDirectInputKeyboardDevice->Unacquire()))
	 return false;
}
...
//	 g_LogFile.OutPutPlainText("SetDataFormat FAILED!!!","3","990000",true);
 ...
 //if(FAILED(m_pDirectInputKeyboardDevice->SetCooperativeLevel(m_hWnd,DISCL_FOREGROUND | DISCL_EXCLUSIVE)))
 {
...
//	 g_LogFile.OutPutPlainText("SetDataFormat FAILED!!!","3","990000",true);
 ...
 //if(FAILED(m_pDirectInputMouseDevice->SetCooperativeLevel(m_hWnd,DISCL_NONEXCLUSIVE|DISCL_FOREGROUND)))
