...
void CParser::SetAll(char* pSet)
{
	m_pStart=m_pTravel=pSet;
	m_iSize=sizeof(pSet)+1;
}

char* CParser::NextLine(void)
{
	if (m_pTravel[0]==NULL)
	return NULL;
...
char* CParser::NextWordIgnoreComments(void)
{
...
// NO alloc
void CParser::GetString(char*& get) // ignore comments etc.
{
	if (m_pTravel[0]==NULL)
	{
		get[0]=NULL;
...
void CParser::GetStringIgnoreSpace(char*& get)
{
	if (m_pTravel[0]==NULL)
	{
		get[0]=NULL;
...
// ignore comments etc.
// assumes string is starting at m_pTravel[0]
void CParser::GetStringAndAlloc(char*& get) // ignore comments etc.
{
	char* pTemp;
	int iCount=0;
...
void CParser::RemoveParenthesis(char*& rem)
{
...
bool CParser::GetNextCurlyBracket(char*& get)
{
	char *pTemp1, *pTemp2;
