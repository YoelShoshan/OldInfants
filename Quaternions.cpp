...
void CQuaternion::CreateMatrix(CMatrix &m)
{
...
void CQuaternion::CreateFromMatrix(float *pTheMatrix, int rowColumnCount)
{
	...
	else 
	{
		...
		{	
			...
			scale  = (float)sqrtf( 1.0f + pMatrix[0] - pMatrix[5] - pMatrix[10] ) * 2.0f;
		...
		else if ( pMatrix[5] > pMatrix[10] ) 
		{
			...
			scale  = (float)sqrtf( 1.0f + pMatrix[5] - pMatrix[0] - pMatrix[10] ) * 2.0f;
		...
		{	
			...
			scale  = (float)sqrtf( 1.0f + pMatrix[10] - pMatrix[0] - pMatrix[5] ) * 2.0f;
