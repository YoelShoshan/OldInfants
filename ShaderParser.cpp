...
{
	g_LogFile.OutPutPlainText("Building File List:","3","000000",true);
	m_ShaderCacheFileVector.reserve(300);
	g_FileManager.FillFileList(Path,What,m_ShaderCacheFileVector);
	
}

void CShaderParser::DebugDrawFileList(void)
{
	int TextY=0;

	for (int i=0;i<m_ShaderCacheFileVector.size();i++)
	{
		glRasterPos2f(7.f, 90.f - (TextY*10.f));
		g_Fonts.glPrint(m_ShaderCacheFileVector[i].c_str());
		TextY++;
	}
}
...
void CShaderParser::BuildCacheMap(void)
{
	...
	for (int i=0;i<m_ShaderCacheFileVector.size();i++)
	{
		g_LogFile.OutPutPlainText("<li> ","3","000000",false);
		g_LogFile.OutPutPlainText("Caching ","3","000000",false);
		g_LogFile.OutPutPlainText(m_ShaderCacheFileVector[i].c_str(),"3","000000",false);
...
CShader* CShaderParser::ParseShader(char* ShaderName)
{
	std::ifstream t_ScriptFile;

	CShader*     pShader;
	CShaderPass* pShaderPass;
					...
					if (strcmp(s1,"map")==0)
					{
						...
						if (strcmp(s2,"$whiteimage")==0)
						{
							if (iCurrentPassNum==0) // TODO:ASK SOMEONE IF THIS IS HACKY OR THE SOLUTION!!!
							{
								...
								pShaderPass->AddNoAnimTexture("ceImages/white.jpg",!bNoMipMaps,false);
