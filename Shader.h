#ifndef YOEL_SHADER_H
#define YOEL_SHADER_H
...
public:
	...
	inline UINT CShaderPass::GetRGBGen(void) const
	{
		return m_uiRGBGen;
	}
	...
	inline UINT CShaderPass::GetALPHAGen(void) const
	{
		return m_uiALPHAGen;
	}
	...
	inline DWORD CShaderPass::GetSrcBlend(void) const
	{
		return m_dwSrcBlend;
	}

	inline DWORD CShaderPass::GetDstBlend(void) const
	{
		return m_dwDstBlend;
	}


	UINT GetTexID(UINT uiNum); 

	//void CalcActiveFrame(void);    

	
	inline void CShaderPass::CalcActiveFrame(float fTimeFake)
	{	
...
CSmartPointer<CTexture> m_spTextures[MAX_ANIM_TEX];
...
class CShader: public CReferenceCount
{
public:
	...
	inline int CShader::GetPassCount() const
	{
		return m_uiPassesNum; 
	}
	
	
	 void AddPass(CShaderPass* pSP); 
	 void ChangePass(CShaderPass* pSP,int iPassNum); 


	inline CShaderPass* CShader::GetPass(const UINT iPass) const
	{
	 ...
	 void SetSurfaceParm(DWORD dwSurfaceParm) { m_dwSurfaceParm = dwSurfaceParm;};
	 DWORD GetSurfaceParm(void) {return m_dwSurfaceParm;};
	 ...
	 void SetAllBlended(bool bAllBlended) { m_bAllBlended = bAllBlended;};
	 bool GetAllBlended(void) { return m_bAllBlended;}
	 ...
	 SDeformVertexesObject* GetDVObject(void) { return &m_DVObject;};

	 bool GetIsDVUsed(void) { return m_bDVUsed;};
...
protected:
	 ...
	 CShaderPass**   m_ppPasses; 

};

#define SHADER_MAN_MAX_SHADERS 3000

#define st_ShaderManager CSingleton<CShaderManager>
...
public:
	...
	int  FindShader(char* cpShaderName);
	int  FindFirstFreeShader(void);
	
	CShader* PushWithoutCheck(char* ShaderName);

	CShader* Load(char* ShaderName);

	bool Unregister(CShader* pShader);

	//CShader* GetShader(UINT uiShaderID);
...
private:
	...
	//CShader ** m_ppShaders;
	CSmartPointer<CShader>* m_sppShaders;
};
